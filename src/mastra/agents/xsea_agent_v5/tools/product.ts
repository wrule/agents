...
当需要创建产品时，调用此工具
  ...
  execute: async ({ context, resourceId: cookie }) => {
    ...
    return await toolExecute('创建产品工具', context, async (context) => {
      const { data } = await thttp(cookie).post(`xsea/workspace/save`, {
        name: context.name,
        remark: context.desc,
      });
      const { object } = data;
      return {
        success: true,
        url: `${process.env.XSEA_V5}/${envId}/product/business/${object?.id}/overview?tab=0`,
      };
    });
  },
});
...
当需要 查询|解释|分析 某个产品的时候调用此工具
  `.trim(),
  inputSchema: z.object({
    query: z.string().describe('产品的查询短语，自动生成，productId优先'),
  }),
  outputSchema: z.object({
    success: z.boolean().describe('调用是否成功'),
    prompt: z.string().optional().describe('向用户解释调用结果的prompt'),
